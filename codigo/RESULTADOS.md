...
| 7 | OSMnx (grafo vial real) | `test_osmnx.py` / `test_osmnx_small.py` | ❌ Caído en el momento de la prueba | Se verificó una **caída real de la infraestructura de Overpass**, no un bloqueo por nuestro uso: tanto el servidor principal (`overpass-api.de`) como un espejo independiente (`overpass.kumi.systems`) fallan por timeout/connection refused. Overpass sí respondió bien a una consulta liviana al principio de la sesión (ver fuente 2 arriba), así que el código es correcto; es un problema de disponibilidad del servicio público. **Plan B confirmado**: extracto offline `download.geofabrik.de/south-america/chile-latest.osm.pbf` (headers verificados, accesible) para no depender de la disponibilidad de Overpass el día de la presentación |
...
Ver `evidencia/`: `open_meteo_response.json`, `overpass_response.json`, `usgs_response.json`, `conaf_probabilidad_ignicion.geojson`, `conaf_areas_protegidas.geojson`.
